...
Public Class ScheduleBuilder
    ...
    Private Sub dgScheduler_CellMouseDoubleClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgSchedule.CellMouseDoubleClick
        ...
        'dtSchedule has a row for each date in the schedule and a column for each match
        'loop through the table from bottom row to top
        'once we hit the skipped row, move columns from row + 1 to row 
        ...
        'rebuild the columns into dwork table (dt) so we can build the csv with a row for each match and a column for each date
        ...
        'rebuildDates(oHelper.dsLeague.Tables("dtSchedule"))

        Dim x = Main.cbLeagues.SelectedItem.ToString
        Dim xx = Main.cbLeagues.SelectedItem.ToString.IndexOf("(") + 1
        Dim sfilename As String = Main.cbLeagues.SelectedItem.ToString.Substring(Main.cbLeagues.SelectedItem.ToString.IndexOf("(") + 1, 4) &
                               "_" & Main.cbLeagues.SelectedItem.ToString.Substring(0, Main.cbLeagues.SelectedItem.ToString.IndexOf("(") - 1) & "_Schedule.csv"
        'col.ColumnName 
