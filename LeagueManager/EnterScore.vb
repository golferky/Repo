...
Public Class frmEnterScore
    Dim oHelper As Helper = Main.oHelper
    ...
    Private Sub EnterScore_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        For i As Integer = 1 To oHelper.rLeagueParmrow("Holes").ToString
    ...
    Public Sub BuildScoringControls()
        ...
        With oHelper
            ...
            ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft, 150, "", "Player")
            ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft, 100, "", "Date")
            ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft, 150, "", "Course")
            ileft = 30
            itop = 60
            Dim x = frmChooseScores.txtPlayer.Text
            ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft, 150, "Player", frmChooseScores.txtPlayer.Text)
            ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft, 100, "Date", frmChooseScores.txtDate.Text)
            ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft, 150, "Course", frmChooseScores.txtCourse.Text)
            ileft = 30
            itop = 150
            ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft, 100, "", "Scoring Method", "B")
            ileft = 30
            itop = 180
            'Dim xx As String = Me.gbScoring.Controls.Find("Method", True).OfType(Of Label).First().Text
            ileft = .CreateTextbox(Me.gbScoring.Controls, itop, ileft, 100, "Method", "", "")
            itop = 150
            For i As Integer = 1 To .rLeagueParmrow("Holes").ToString
                ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft, 30, "Hole" & i, i, "B")
                Dim A As Label = Me.gbScoring.Controls.Find("Hole" & i, True).OfType(Of Label).First()
                'MsgBox("lb" & A.TabIndex)
                .CreateNumericTextbox(Me.gbScoring.Controls, itop + 30, ileft - 30, 30, "Hole" & i, String.Empty)
                'MsgBox(B.Name & " " & B.TabIndex)
                If .rLeagueParmrow("Holes").ToString > 9 Then
                    If i = 9 Then
                        ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft + 5, 40, "Out", "Out", "B")
                        .CreateLabel(Me.gbScoring.Controls, itop + 3 + 30, ileft - 35, 30, "OutScore", String.Empty)
                    ElseIf i = 18 Then
                        ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft + 5, 40, "In", " In", "B")
                        .CreateLabel(Me.gbScoring.Controls, itop + 3 + 30, ileft - 35, 30, "InScore", String.Empty)
                    End If
                End If
            Next
            'Create Total 
            ileft = .CreateLabel(Me.gbScoring.Controls, itop, ileft + 5, 40, "Total", "Total", "B")
            .CreateLabel(Me.gbScoring.Controls, itop + 3 + 30, ileft - 35, 30, "TotScore", String.Empty)
        End With
    ...
    Public Sub Calc(stb As MaskedTextBox)
        ...
        If scores(sHole) <> String.Empty Then
            bcorrection = True
            If oHelper.rLeagueParmrow("Holes").ToString > 9 Then
                If sHole < 9 Then
...
#End Region
    ...
    Private Sub btnUpdatesScoring_Click(sender As System.Object, e As System.EventArgs) Handles btnUpdatesScoring.Click
        ...
        oHelper.CSV2DataTable(edt, "Scores.csv")
        ...
        ohelperlv.BuildScoreLV(lv1, "Player|Date")
