...
Public Class Main
    Dim cVersion = "Version : 2029.01.22"
    ...
    Public sini As String = "Leaguemanager.ini"
    Sub testcode()
        ...
        Dim sRowFilter = String.Format("Player = '{0}' and Date < '{1}' and Method <> '' and date not in ('{2}')", "Gary Scudder", "2019-09-03", String.Join("','", lignoreDates))
        sRowFilter &= String.Format(" AND (Out_Gross <> '' or In_Gross <> '')")
        Dim dt = dsLeague.Tables("dtScores").Select(sRowFilter, "Date desc").CopyToDataTable
        Dim dvScores As New DataView(dsLeague.Tables("dtScores"))
        dvScores.RowFilter = String.Format("Player = '{0}' and Date < '{1}' and Method <> '' and date not in ('{2}')", "Gary Scudder", "2019-09-03", String.Join("','", lignoreDates))
        dvScores.RowFilter &= String.Format(" AND (Out_Gross <> '' or In_Gross <> '')")
        dvScores.Sort = "Date desc"
        Dim dt1 As DataTable = dvScores.ToTable
        Dim dr = dt.Rows.Cast(Of DataRow).Take(5)
    End Sub
    Private Sub Main_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            oHelper = New Helper
            oHelper.bloghelper = True
            For Each col As DataColumn In dsLeague.Tables("dtScores").Columns
                If col.DataType <> GetType(String) Then
                    If col.ColumnName.Contains("Points") Then Continue For
                    oHelper.LOGIT(String.Format("changing col {0} from {1} to string", col.ColumnName, col.DataType))
                    col.DataType = GetType(String)
                End If
            Next
            Me.Show()

#Region "Display on secondary monitor"
...
#End Region
            ...
            oHelper.status_Msg(lblProcessMsg, Me)
            oHelper.sVersion = "No Version"
            'load cookie settings

            '2020 testing use 2020_Leaguemanager.ini
            sini = "2020_" & sini
            sini = sDocs & "\" & sini
            If IO.File.Exists(sini) Then
                Using sr As New IO.StreamReader(sini, False)
                    Do
                        ...
                        Select Case slineparts(0)
                            ...
                            Case "DateOverlapReminder"
                                ...
                                End If
                            Case "Version"
                                oHelper.sVersion = slineparts(1)
                        End Select
                    Loop
                End Using
            End If
            Dim spgmVersion As String = cVersion.ToString.Replace(".", "-").Substring(cVersion.ToString.IndexOf(":") + 1)
            If Not oHelper.sVersion = spgmVersion Then
                Dim smsg = String.Format("Program version mismatch{0}Expected {1} and found {2}{0}Youre missing the latest files or latest version{0}Do you want to find a different folder?", vbCrLf, cVersion, oHelper.sVersion)
                Dim mbr = MessageBox.Show(smsg, "Version Checker", MessageBoxButtons.OKCancel)
                ...
                Dim dialog As New FolderBrowserDialog With
                                ...
                                .SelectedPath = oHelper.sFilePath,
    ...
    Sub BuildTablesForLeague()
        ...
        oHelper.thisCourse = dsLeague.Tables("dtCourses").Rows.Find(oHelper.rLeagueParmrow("Course"))
